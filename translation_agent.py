...
def translate_text_with_gemini(text: str, target_lang: str, source_lang: str = 'en') -> tuple[str, float]:
    ...
    for attempt in range(max_retries):
        try:
            ...
            prompt = (
                f"Translate this text from {source_lang} to {lang_name} ({target_lang}). "
                f"Use {script} script. Return ONLY a JSON object with 'translated_text' and 'confidence_score' (0.0-1.0). "
            ...
            if not cleaned_response_text.endswith('}') and not cleaned_response_text.endswith('}'):
                logger.warning(f"Response appears truncated for {target_lang}: {cleaned_response_text[-50:]}")
                # Try to fix common truncation issues
                if '"confidence_score":' in cleaned_response_text and not cleaned_response_text.endswith('}'):
                    # Find the last complete confidence score and close the JSON
                    import re
                    match = re.search(r'"confidence_score":\s*([0-9.]+)', cleaned_response_text)
                    if match:
                        confidence_value = match.group(1)
                        # Reconstruct the JSON properly
                        text_match = re.search(r'"translated_text":\s*"([^"]*)"', cleaned_response_text)
                        if text_match:
                            translated_text_value = text_match.group(1)
                            cleaned_response_text = f'{{"translated_text": "{translated_text_value}", "confidence_score": {confidence_value}}}'
                            logger.info(f"Reconstructed truncated JSON for {target_lang}")
            ...
            try:
                ...
                # Try to extract translation from malformed response as fallback
                import re
                text_match = re.search(r'"translated_text":\s*"([^"]*)"', cleaned_response_text)
                if text_match:
                    translated_text = text_match.group(1)
                    confidence = 0.7  # Lower confidence due to parsing issues
                    logger.info(f"Extracted translation from malformed JSON for {target_lang}: {translated_text}")
                else:
